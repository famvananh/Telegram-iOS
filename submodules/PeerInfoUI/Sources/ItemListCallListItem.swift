...
public class ItemListCallListItem: ListViewItem, ItemListItem {
    ...
    public init(presentationData: ItemListPresentationData, dateTimeFormat: PresentationDateTimeFormat, messages: [Message], sectionId: ItemListSectionId, style: ItemListStyle, displayDecorations: Bool = true, currentDate: Int32? = nil) {
...
//            let titleText = stringForDate(timestamp: earliestMessage.timestamp, strings: item.presentationData.strings)
            let earliestMessage = item.currentDate ?? item.messages.sorted(by: {$0.timestamp < $1.timestamp}).first!.timestamp
            let titleText = stringForDate(timestamp: earliestMessage, strings: item.presentationData.strings)
